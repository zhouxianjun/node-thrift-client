...
module.exports = class ThriftClient extends EventEmitter {
    constructor(providerFactory, loadBalance = new ThriftClient.loadBalance.RandomLoadBalance()) {
        ...
        this.$prop.providerFactory = providerFactory;
        this.$prop.loadBalance = loadBalance;
    }
    ...
    getService(service) {
        if (Reflect.getPrototypeOf(service) === ReferenceBean) {
            return this.$prop.services.get(service);
        }
        return null;
    }
    addService(service) {
        let obj = service.instance(this.$prop.providerFactory, this.$prop.loadBalance);
        if (obj instanceof ReferenceBean) {
            logger.info('add service %s:%s', obj.service, service);
            this.$prop.services.set(service, obj);
        }
        return this;
    }
    delService(service) {
        if (Reflect.getPrototypeOf(service) === ReferenceBean) {
            ...
            this.$prop.services.delete(service);
        }
        return this;
    }
