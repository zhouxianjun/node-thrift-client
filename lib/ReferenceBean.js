...
module.exports = class ReferenceBean {
    constructor(providerFactory, type, loadBalance = RandomLoadBalance) {
            ...
            if (method != 'constructor' &&
                ...
                Reflect.set(new.target.prototype, method, new Proxy(new.target.prototype[method], {
                    apply(target, that, args) {
                        let address = providerFactory.allServerAddressList(that.service, that.version, type);
                        let invoker = loadBalance.selector(address, target.name);
                        let defer = Q.defer();
                        invoker.invoker(target.name, args).then(result => {
