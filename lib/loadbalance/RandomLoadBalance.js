...
module.exports = class RandomLoadBalance extends require('./AbstractLoadBalance') {
    static doSelect(invokers, method) {
        ...
        for (let i = 0; i < length; i++) {
            let weight = RandomLoadBalance.getWeight(array[i]);
            totalWeight += weight; // 累计总权重
            if (sameWeight && i > 0 && weight != RandomLoadBalance.getWeight(array[i - 1])) {
        ...
        if (totalWeight > 0 && !sameWeight) {
            ...
            for (let invoker of invokers.values()) {
                offset -= RandomLoadBalance.getWeight(invoker);
                if (offset < 0) {
                    return invoker;
                }
            }
