...
module.exports = class PoolInvoker extends require('./AbstractInvoker') {
    ...
    invoker(method, ...args) {
        logger.debug(`%s:%s invoker:%s args:`, prop.service, prop.address, method, args);
        logger.debug('pool.getMaxPoolSize() = %d', prop.pool.getMaxPoolSize());
        logger.debug('pool.getPoolSize() = %d', prop.pool.getPoolSize());
        logger.debug('pool.availableObjectsCount() = %d', prop.pool.availableObjectsCount());
        let defer = Q.defer();
